...
public:
	...
	static void LoadScene(Scene* ptr, const std::string& filename);
	// deserialize path
	// load stuff and push into *this->mEntities

	//serializationManager->SaveScene(*this); TODO
	static void SaveScene(Scene* ptr);
	...
	static void LoadGameState(GameState* ptr, const std::string& filename);

	//serializationManager->SaveGameState(*this); TODO
	static void SaveGameState(GameState* ptr);

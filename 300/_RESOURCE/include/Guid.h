...
struct crc
{
    ...
    static constexpr std::array<T, 256> ms_Table =
    {
...
public:
    ...
    Base(const std::uintptr_t& _typeid) : m_typeid(_typeid) {};

    template<typename T>static inline std::uintptr_t type_id() {
        return reinterpret_cast<std::uintptr_t>(&type_id<T>);
    }


    std::uintptr_t m_typeid;
};
...
struct uid {
    ...
    */
    ...
    unsigned getstringUid(std::string name) {
